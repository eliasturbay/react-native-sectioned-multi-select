...
class SectionedMultiSelect extends PureComponent {
  ...
  ModalContentWrapper = ({ children }) => {
    ...
    return (
      <Component
        style={[{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)' }, styles.modalWrapper]}
        accessible={false}
      >
        {children}
      </Component>
      ...
      <View>
        ...
        {!hideSelect && (
          <TouchableWithoutFeedback
            ...
            testID={`dropdown-${items && items.length ? items[0].name : 'without-items'}`}
            accessibilityLabel={`dropdown-${items && items.length ? items[0].name : 'without-items'}`}
          >
            <View
              style={[
                {
